...
window.updateAudioIcon = function (state) {
    if (state === 'error') { audioBtn.textContent = "AUDIO: ERROR"; }
    else {
...
if (window.location.protocol !== "https:" && window.location.hostname !== "localhost" && window.location.hostname !== "127.0.0.1") {
    try {
        delete Navigator.prototype.gpu; Object.defineProperty(Navigator.prototype, "gpu", { get: () => undefined, configurable: true, });
    } catch (e) { }
...
init().then(() => {
    ...
    if (gameCanvas) { gameCanvas.id = 'game-canvas'; document.querySelector('.canvas-inner').appendChild(gameCanvas); }
});
let audioCtx = null;
...
function initAudio() {
    ...
    audioInitPromise = new Promise(async (resolve) => {
        ...
        if (audioCtx.audioWorklet) {
            ...
            try {
                ...
                resolve(true);
            } catch (err) { console.error("Failed to load AudioWorklet:", err); window.updateAudioIcon('error'); resolve(false); }
        } else {
            ...
            try {
                ...
                scriptNode.onaudioprocess = function (audioProcessingEvent) {
                    ...
                    for (let i = 0; i < bufferSize; i++) { channel[i] = buffer[i]; }
                };
                const dummyOsc = audioCtx.createOscillator();
                dummyOsc.connect(scriptNode);
                scriptNode.connect(audioCtx.destination);
                resolve(true);
            } catch (err) { console.error("Failed to load ScriptProcessorNode fallback:", err); window.updateAudioIcon('error'); resolve(false); }
        }
    });
...
btn.addEventListener('click', async (e) => {
    ...
    if (isAudioPlaying) { window.updateAudioIcon('on'); btn.classList.add('active'); }
    else { window.updateAudioIcon('off'); btn.classList.remove('active'); }
});
